...
import { SupplierOrderDetailsPage } from '../supplier-order-details/supplier-order-details';


@Component({
  selector: 'page-supplier-orders',
...
export class SupplierOrdersPage {
    ...
    this.storage.ready().then(() => {
      this.storage.get('loggedInUserDetails').then((loggedInUserDetails) => {
        if (loggedInUserDetails == null) {
          ...
          loading.present();
          this.datalink.getPlacedOrders(this.userid, "Pending", "Supplier")
            .subscribe(result => {
    ...
    this.storage.ready().then(() => {
      this.storage.get('loggedInUserDetails').then((loggedInUserDetails) => {
        if (loggedInUserDetails == null) {
          ...
          this.userid = this.UDetails.userid;
          loading.present();
          this.datalink.getPlacedOrders(this.userid, "Delivered", "Supplier")
            .subscribe(result => {
              loading.dismiss().catch(() => { });
              if (result[0] === "200") {
                this.originalorders = result[1];
    ...
    this.storage.ready().then(() => {
      this.storage.get('loggedInUserDetails').then((loggedInUserDetails) => {
        if (loggedInUserDetails == null) {
          ...
          this.userid = this.UDetails.userid;
          loading.present();
          this.datalink.getPlacedOrders(this.userid, "Cancelled", "Supplier")
            .subscribe(result => {
              loading.dismiss().catch(() => { });
              if (result[0] === "200") {
                this.originalorders = result[1];
  ...
  GotoOrderDetails(orderid) {
    this.navCtrl.push(SupplierOrderDetailsPage, { orderid });
  }
